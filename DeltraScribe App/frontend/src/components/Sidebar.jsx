      ...
      <div className={`p-8 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} mb-6 relative`}>
        {!isCollapsed && (
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-white rounded-2xl shadow-lg shadow-deltascribe-emerald/5 border border-gray-50">
              <Logo className="w-7 h-7 text-deltascribe-emerald" color="currentColor" />
            </div>
            <div>
              <h1 className="text-xl font-black text-slate-800 tracking-tight leading-none uppercase">DeltaScribe</h1>
              <span className="text-[10px] font-bold text-deltascribe-emerald tracking-widest uppercase opacity-70">Audit Engine</span>
            </div>
          </div>
        )}
        {isCollapsed && (
           <div className="p-2 bg-white rounded-2xl shadow-lg shadow-deltascribe-emerald/5 border border-gray-50 scale-110">
              <Logo className="w-6 h-6 text-deltascribe-emerald" color="currentColor" />
           </div>
        )}
        ...
        <button 
          onClick={onToggle}
          className={`absolute ${isCollapsed ? '-right-3' : '-right-3'} top-10 p-1.5 bg-white border border-gray-100 rounded-lg shadow-md text-gray-400 hover:text-deltascribe-emerald hover:scale-110 transition-all z-30`}
        >
          ...
          return (
            <Link
              ...
              className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} p-3.5 rounded-2xl transition-all group ${
                isActive 
                ? 'bg-deltascribe-gradient text-white shadow-xl shadow-deltascribe-emerald/20' 
                : 'text-slate-400 hover:text-deltascribe-emerald hover:bg-emerald-50'
              }`}
              title={isCollapsed ? item.label : ''}
            >
              <div className="flex items-center space-x-3">
                <item.icon className={`w-5 h-5 ${isActive ? 'text-white' : 'group-hover:text-deltascribe-emerald'}`} />
