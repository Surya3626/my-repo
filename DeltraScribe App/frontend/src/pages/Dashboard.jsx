...
const Dashboard = () => {
    ...
    const statCards = metrics ? (isTaskCentric ? [
        { 
            ...
            isUp: true, 
            icon: CheckSquare, 
            color: 'from-deltascribe-navy to-indigo-600',
            bg: 'bg-indigo-50'
        },
        { 
            label: 'Open Tasks', 
        ...
        { 
            ...
            isUp: true, 
            icon: Bug, 
            color: 'from-deltascribe-emerald to-emerald-600',
            bg: 'bg-emerald-50'
        },
        { 
            label: 'Open Bugs', 
    ...
    if (isLoading) return <div className="p-8 flex items-center justify-center min-h-[60vh]"><Logo className="animate-spin w-8 h-8 text-deltascribe-emerald" /></div>;
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight">DeltaScribe Intelligence</h1>
                    <p className="text-gray-500 font-medium">Monitoring system audit flow and task precision.</p>
                </div>
                <div className="flex items-center space-x-3">
                    ...
                    {isAdmin && (
                        <button 
                            ...
                            className="bg-deltascribe-gradient text-white font-black py-2.5 px-6 rounded-xl flex items-center space-x-2 shadow-lg shadow-deltascribe-emerald/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                        >
                ...
                <div className="bg-white rounded-[40px] border border-gray-100 p-8 shadow-sm overflow-hidden relative group">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-deltascribe-emerald uppercase tracking-widest opacity-70">
                        ...
                        <button 
                            ...
                            className="text-xs font-bold text-deltascribe-navy hover:underline flex items-center space-x-1"
                        >
                            <span>View All</span>
                                ...
                                {(isTaskCentric ? recentTasks : recentBugs).map((item) => (
                                    <tr key={item.id} className="group hover:bg-gray-50/50 transition-colors">
                                        <td className="py-4 pl-2">
                                            <span className="text-xs font-black text-deltascribe-emerald">{item.jtrackId || (isTaskCentric ? `T-${item.id}` : `B-${item.id}`)}</span>
                                        </td>
                                        <td className="py-4">
                                        ...
                                        {isAdmin && (
                                            <td className="py-4 text-right">
                                                <button 
                                                    ...
                                                    className="p-2 rounded-xl bg-gray-50 text-gray-400 hover:bg-deltascribe-emerald hover:text-white transition-all transform group-hover:scale-110"
                                                >
