...
const Reports = () => {
    ...
    if (isLoading && !metrics) return <div className="p-8 text-center text-deltascribe-navy font-bold"><Logo className="animate-spin inline-block mr-2 w-6 h-6" color="currentColor" />Generating Analytical Report...</div>;
        ...
        <div className="space-y-8 animate-fade-in pb-12">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm">
                    <div className="flex items-center space-x-2 px-3">
                        <Calendar className="w-4 h-4 text-deltascribe-emerald" />
                    ...
                    <input 
                        ...
                        className="bg-gray-50 border border-transparent hover:border-gray-200 rounded-xl py-2 px-3 text-sm font-bold text-gray-700 outline-none focus:bg-white focus:border-deltascribe-emerald transition-all"
                    />
                    ...
                    <button 
                        ...
                        className="btn-deltascribe py-2 px-6 ml-2 text-sm shadow-md"
                    >
                        Apply Filter
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex items-center space-x-2 text-deltascribe-navy font-bold text-xs">
                        <Target className="w-4 h-4" />
                        <span>{isTaskCentric ? 'Total Tasks Created' : 'Total Bugs & Linked Tasks'}</span>
                    </div>
                </div>

                <div className="bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm relative overflow-hidden group text-deltascribe-navy">
                    <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:scale-110 transition-transform">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        {(!isTaskCentric ? ['OPEN', 'IN_PROGRESS', 'RESOLVED', 'VERIFIED', 'CLOSED', 'BLOCKED'] : ['OPEN', 'IN_PROGRESS', 'RESOLVED', 'VERIFIED', 'CLOSED']).map((status) => (
                            <div key={status} className="bg-gray-50 p-6 rounded-3xl border border-gray-100 text-center hover:shadow-md transition-shadow">
                                <p className="text-4xl font-black text-deltascribe-navy mb-2">
                                ...
                                filteredPerformanceData.map(([dev, assignedCount], i) => (
                                    <div key={i} className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
                                        <div className="flex items-center space-x-4 mb-4">
                                            <div className="w-12 h-12 rounded-2xl bg-deltascribe-navy text-white flex items-center justify-center font-black shadow-md">
                                                {dev.charAt(0).toUpperCase()}
                                            </div>
                                            <div>
                                                <p className="text-base font-black text-gray-800">{dev}</p>
                                                <p className="text-[10px] text-deltascribe-emerald font-bold uppercase tracking-widest">Developer</p>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="bg-white p-3 rounded-xl border border-gray-100/50 text-center">
                                                <p className="text-2xl font-black text-deltascribe-emerald">{metrics.bugsSolvedByDev?.[dev] || 0}</p>
                                ...
                                filteredPerformanceData.map(([tester, raisedCount], i) => (
                                    <div key={i} className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex flex-col justify-between">
                                        <div className="flex items-center space-x-4 mb-6">
                                            <div className="w-12 h-12 rounded-2xl bg-deltascribe-emerald text-white flex items-center justify-center font-black shadow-md">
                                                {tester.charAt(0).toUpperCase()}
                                            </div>
                                            <div>
                                                <p className="text-base font-black text-gray-800 line-clamp-1">{tester}</p>
                                                <p className="text-[10px] text-deltascribe-navy font-bold uppercase tracking-widest">Quality Assurance</p>
                                            </div>
                                        </div>
                                        <div className="flex space-x-4 bg-white p-4 rounded-2xl border border-gray-100/50">
                                            ...
                                            <div className="flex-1 text-center">
                                                <p className="text-2xl font-black text-red-500">{metrics.invalidBugsRaisedByTester?.[tester] || 0}</p>
