			...
			for (Cookie cookie : cookies) {
				if ("JWT_TOKEN".equals(cookie.getName())) {
					...
					try {
						...
						if (user != null) {
							if (user.getRoles().contains(com.example.devtrack.model.Role.ADMIN)) {
								return "redirect:/";
							}else if (user.getRoles().contains(com.example.devtrack.model.Role.TESTING)) {
	...
	public String createToken(@RequestParam String userId, @RequestParam String password,
		...
		try {
			...
			if (userDetails.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"))) {
				return "redirect:/";
			} else if (userDetails.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_TESTING"))) {
				return "redirect:/bug-manager";
