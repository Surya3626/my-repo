                ...
                <div className="flex bg-gray-100 p-1.5 rounded-2xl w-full md:w-auto">
                    <button 
                        ...
                        className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                            filterStatus === 'PENDING' ? 'bg-white text-tata-pink shadow-sm' : 'text-gray-400 hover:text-gray-600'
                        }`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                            filterStatus === 'COMPLETED' ? 'bg-white text-tata-pink shadow-sm' : 'text-gray-400 hover:text-gray-600'
                        }`}
                    >
                        Closed Reviews
                    </button>
                ...
                {filteredReviews.length === 0 ? (
                    ...
                    >
                        ...
                        <div className="flex flex-row md:flex-col items-center md:items-start gap-3 md:gap-1 min-w-[120px]">
                            <span className="text-[10px] font-black text-tata-pink uppercase tracking-widest bg-pink-50 px-3 py-1 rounded-full border border-pink-100">
                                {item.jtrackId || `T-${item.id}`}
                            </span>
                            <div className="flex items-center space-x-1">
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">TYPE:</span>
                                <span className="text-xs font-black text-tata-purple uppercase">{item.type?.name}</span>
                            </div>
                        </div>

                        {/* Title and Dev */}
                        <div className="flex-1 text-center md:text-left">
                            <h3 className="text-lg font-black text-gray-800 group-hover:text-tata-pink transition-colors mb-1">
                                {item.title}
                            </h3>
                        ...
                        <div className="flex items-center space-x-4 min-w-[180px] justify-end">
                            ...
                            {filterStatus === 'PENDING' ? (
                                <button className="p-3 rounded-2xl bg-gray-50 text-gray-400 group-hover:bg-tata-pink group-hover:text-white transition-all transform group-hover:scale-110 shadow-sm border border-transparent group-hover:border-tata-pink/20">
