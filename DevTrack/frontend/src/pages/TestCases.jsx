...
import { 
    Plus, Search, Filter, TestTube2, AlertCircle, ChevronRight, Activity, 
...
const TestCases = () => {
    ...
    const handleExportToExcel = () => {
        ...
        XLSX.writeFile(wb, `DevTrack_TestCases_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    if (isLoading) return <div className="p-8 text-center text-tata-pink font-bold animate-pulse">Initializing QA Engine...</div>;
                        ...
                        <div key={task.id} className="bg-white rounded-[32px] border border-gray-100 overflow-hidden shadow-sm transition-all hover:shadow-md">
                            ...
                            >
                                <div className="flex items-center space-x-4">
                                    <div className={`p-3 rounded-2xl ${isExpanded ? 'bg-tata-purple text-white' : 'bg-gray-100 text-gray-400'}`}>
                                        <Layout className="w-5 h-5" />
                                    </div>
                                    <div>
                                        <div className="flex items-center space-x-2">
                                            <span className="text-[10px] font-black text-tata-purple uppercase tracking-widest">{task.jtrackId}</span>
                                ...
                                <div className="border-t border-gray-50 p-4 bg-white grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {taskTCs.map(tc => (
                                        <div 
                                            ...
                                            className="p-6 rounded-3xl border border-gray-50 hover:border-tata-pink/30 hover:shadow-xl transition-all cursor-pointer group"
                                        >
                                            ...
                                            <h4 className="text-sm font-black text-gray-800 group-hover:text-tata-pink transition-colors line-clamp-2 mb-2">{tc.title}</h4>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
                                                <div className={`flex items-center space-x-1 ${
                                                    ...
                                                    tc.status === 'FAIL' ? 'text-tata-pink' :
                                                    tc.status === 'IN_PROGRESS' ? 'text-tata-purple' :
                                                    tc.status === 'PENDING' ? 'text-orange-400' : 'text-gray-400'
                                                }`}>
                                                    {tc.status === 'PASS' ? <CheckCircle2 className="w-3 h-3" /> : <Activity className="w-3 h-3" />}
