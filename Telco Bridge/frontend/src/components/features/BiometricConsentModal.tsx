...
export const BiometricConsentModal: React.FC<BiometricConsentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-fade-in">
      <div className="clay-modal p-6 text-white w-full max-w-sm shadow-2xl space-y-5 text-center relative">
        {/* Header */}
        ...
        <div className="py-4 space-y-4">
          <div className="relative w-24 h-24 mx-auto rounded-full clay-badge-purple flex items-center justify-center">
            {status === 'SUCCESS' ? (
              <CheckCircle2 size={48} className="text-emerald-400 animate-bounce" />
            ) : status === 'SCANNING' ? (
              <Scan size={48} className="text-purple-400 animate-spin" />
            ) : (
              <Fingerprint size={48} className="text-purple-500 dark:text-purple-300 hover:scale-110 transition cursor-pointer" onClick={handleSimulateBiometric} />
            )}
        ...
        {status !== 'SUCCESS' && (
          <button
            ...
            disabled={scanning}
            className="w-full py-3 clay-button-purple text-white text-xs font-extrabold shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
          >
